...
const BottomProgressPeek: React.FC<BottomProgressPeekProps> = ({
  healthData,
  onExpand,
}) => {
  ...
  return (
    <motion.div
      ...
      dragElastic={0.2}
      onDragEnd={(_, info) => {
        if ((info?.offset?.y ?? 0) < -40) onExpand();
      }}
      onClick={onExpand}
    >
      <div className="rounded-full overflow-hidden shadow-2xl border border-white/30 dark:border-white/10 backdrop-blur bg-white/70 dark:bg-black/50">
        {/* Handle bar */}
        <div className="flex justify-center py-2">
          <div className="w-12 h-0.5 bg-gray-400/70 rounded-full" />
        </div>
        {/* Mini summary */}
